...
echo "$($PSQL "SELECT T.name FROM games G LEFT JOIN teams T ON G.winner_id = T.team_id OR G.opponent_id = T.team_id WHERE G.year = 2014 AND G.round = 'Eighth-Final' GROUP BY T.name ORDER BY T.name")"
